...
// once.
const MIN_REFERENCE_CORPUS_BYTES = 100_000; // re-derived 2026-09-02 (work-issues retro) at the final tree: 9 stage files, corpus 136,420 B, largest implement.md 37,358 B, so the property needs a floor above 136,420 - 37,358 = 99,062 -- which 100,000 still provides, but with 938 B of margin, down from the 3,724 B measured on 2026-09-01 at corpus 130,349 / largest 34,073. THAT DECAY IS THE POINT: the margin is consumed by growth in the NON-largest files (triage.md and verify.md took +737 and +1,865 B in this retro alone), so the next stage-file paragraph anywhere but implement.md reds this assertion -- which is the designed behaviour, not a surprise. When it fires, RAISE the floor (strictly tighter, no upper bound moves) and re-derive this comment; do not lower it. Sized against `corpus - largest` rather than the either-largest case, because the top two are 14,529 B apart (implement.md 37,358, verify.md 22,829) -- a flip is not near; cdkd sizes against the flip because its top two are 3,242 B apart. 36,420 B (136,420 - 100,000) of narrative compression headroom remains below the floor
