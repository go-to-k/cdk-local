...
/**
 * The body-side English check, ported for the CI
...
describe('the report', () => {
  ...
  it('carries the fix guidance and the rule pointer', () => {
    ...
    expect(report).toContain('.claude/CLAUDE.md -> Workflow rules -> English only');
...
describe('the RENDERED report contains attacker text (marked, not string shapes)', () => {
  ...
  //
  // `marked` is a devDependency added for this suite: the report is posted as
  // a comment under the repository's own identity by a job holding
  // `issues: write`, so "does the attacker text escape its fence" has to be
  // asked of a real CommonMark renderer, not of a string shape
