...
/**
 * The CI replacement for `.claude/hooks/non-english-text-gate.sh`, whose own
 * coverage lived in `gate-command-recognition.test.sh`.
 *
...
describe('the sidecar allow-list', () => {
  ...
  it('exempts a listed path exactly', () => {
    const allowed = [
      '.claude/hooks/some-gate.test.sh',
      'tests/unit/utils/docker-cmd.test.ts',
    ];
    expect(isAllowlisted('.claude/hooks/some-gate.test.sh', allowed)).toBe(true);
  ...
  it('still blocks a non-listed SIBLING in the same directory', () => {
    const allowed = ['.claude/hooks/some-gate.test.sh'];
  ...
  it('matches EXACTLY -- never a prefix, never a glob', () => {
    const allowed = ['.claude/hooks/some-gate.test.sh'];
    expect(shouldScan('.claude/hooks/some-gate.test.sh.bak', allowed)).toBe(true);
    expect(shouldScan('.claude/hooks/', allowed)).toBe(true);
    expect(shouldScan('x/.claude/hooks/some-gate.test.sh', allowed)).toBe(true);
  });
...
describe('the shipped allow-list and the check\'s own sources', () => {
  ...
  // NO `length > 0` FLOOR, and that is a deliberate difference from the repo
  // this was ported from. This repo's tree carries no non-English
  // writing-system characters in any scannable file, so the shipped list is
  // legitimately EMPTY -- and an empty list is the strictest state there is
  // (it means scan everything). A floor here would force someone to invent an
  // entry, which is a permanent hole, to satisfy a test.
  //
  // The case is therefore about STALENESS only, and it is not vacuous: the day
  // an entry is added it starts guarding it.
  it('no shipped entry names a path that no longer exists', () => {
    const allowed = parseAllowlist(readFileSync(ALLOWLIST_PATH, 'utf8'));
  ...
  it.each([
    ...
    // The body-side half of the family. The two TEST files are exactly where
    // someone pastes a fixture character, and the cost of that is every later
    // PR touching them being unopenable until an allow-list entry is added.
...
function makeRepo(files: Record<string, string>): Repo {
  const dir = mkdtempSync(join(tmpdir(), 'cdkl-noneng-'));
...
describe('end-to-end against a real repository', () => {
  ...
  // The in-case `cleanup()` calls stay -- they keep each case's tmpdirs from
  // piling up across the describe. This is the BACKSTOP: a throw in `build()`
  // or `runCheck` skips the in-case call and leaks the directory, and every
  // one of these holds a git repo. `cleanup` is idempotent (it empties
  // `repos`), so running it twice costs nothing.
  afterEach(cleanup);
  ...
  // WHAT IS NOT COVERED END TO END HERE, stated rather than left to be
  // discovered: the allow-list's PASS half. The port this came from drove the
  // first shipped entry through the real CLI, and this repo's shipped list is
  // empty, so there is no live entry to drive. Writing one in from a test
  // would mean a test mutating the check's own definition on disk.
  //
  // The predicate is covered at unit level with synthetic entries (exact
  // match, no prefix, no glob, sibling still blocked). What this case keeps is
  // the half that can go wrong silently: a file that is NOT allow-listed must
  // block, through the real binary.
  it('blocks a non-listed file through the real CLI', () => {
    const sibling = join('src', 'not-listed.ts');
