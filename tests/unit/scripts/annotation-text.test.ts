...
describe('the rule reaches every checker that emits an annotation', () => {
  it('the set of annotation emitters is exactly the set that folds', () => {
    ...
    // ONE emitter in this repo today. Pinned as a set rather than floored, so
    // adding a second check that writes `::error` is a deliberate edit here and
    // cannot silently join without the shared fold.
  ...
  it.each([['check-pr-non-english-text.ts']])('%s interpolates an offender field ONLY inside a format function', (file) => {
  ...
  it('the fold reaches the family half that emits no `::` at all', () => {
    // `check-gh-body-english.ts` writes no workflow command, so the scan above
    // cannot see it -- but its output reaches the log through
    // `issue-conventions.yml`, in the job that holds `issues: write` and that
    // ANY GitHub user can trigger via `issue_comment`. A private fold there is
