...
describe('release-pr-staleness', () => {
  ...
  function fixture(opts: {
    ...
    const restPrs = openPrs.map((o) => ({
      number: o.number,
      head: { ref: o.headRef, repo: o.fork ? { full_name: 'attacker/cdk-local' } : { full_name: 'go-to-k/cdk-local' } },
      base: { repo: { full_name: 'go-to-k/cdk-local' } },
    }));
...
`;
  ...
  it('fails closed when gh returns no head branch name', () => {
    ...
    const r = run(fixture({ stale: true, armed: true, openPrs: [{ number: 7, headRef: RELEASE_BRANCH, fork: false }], headRefAnswerEmpty: true }));
    expect(r.status).not.toBe(0);
  ...
  it('does not evaluate a head branch name that carries shell metacharacters', () => {
    ...
    // reds this case and ONLY this case (17 others stay green). The probe is
    // the side effect — if the `$(…)` ever runs, the file exists. The run
    // itself is expected to fail, since no such branch exists to fetch.
