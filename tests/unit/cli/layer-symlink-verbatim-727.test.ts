 ...
 * comment above the call had claimed the opposite since it was copied in with the code (cdkd's go-to-k/cdkd#241 backlog).
 *
 ...
 * directory (both found by the go-to-k/cdkd#3118 review rounds of the cdkd twin). The helper
...
describe('copyLayerTreeLastWins merges symlinks last-wins without resolving them (#727)', () => {
  function layer(name: string, build: (dir: string) => void): string {
    const dir = mkdtempSync(join(tmpdir(), `cdkl-727-${name}-`));
    scratch.push(dir);
    mkdirSync(join(dir, 'bin'));
  ...
  function mergedOf(...layers: string[]): string {
    const dest = mkdtempSync(join(tmpdir(), 'cdkl-727-merged-'));
    scratch.push(dest);
  ...
  it('a cyclic directory link terminates the walk, and an absolute link to a host directory never reaches the host', () => {
    const host = mkdtempSync(join(tmpdir(), 'cdkl-727-host-'));
    scratch.push(host);
  ...
  it('an asset dir handed over through a symlink is copied whole (the root is resolved first)', () => {
    ...
    const holder = mkdtempSync(join(tmpdir(), 'cdkl-727-linkroot-'));
    scratch.push(holder);
...
describe('a merge that throws mid-loop removes the tmpdir it had allocated (#727 round 3)', () => {
  ...
  const missing = join(tmpdir(), 'cdkl-727-does-not-exist-' + process.pid);
  const tmpDirsWithPrefix = (prefix: string): string[] =>
    readdirSync(tmpdir()).filter((n) => n.startsWith(prefix));

  it('cdkl invoke', () => {
    const before = tmpDirsWithPrefix('cdkl-invoke-layers-');
    expect(() =>
    ...
    const leaked = tmpDirsWithPrefix('cdkl-invoke-layers-').filter((n) => !before.includes(n));
    expect(leaked, `leaked ${leaked.map((n) => basename(n)).join(', ')}`).toEqual([]);
  });

  it('cdkl start-api', async () => {
    const before = tmpDirsWithPrefix('cdkl-start-api-layers-');
    const tmpDirs = new Set<string>();
    await expect(
    ...
    const leaked = tmpDirsWithPrefix('cdkl-start-api-layers-').filter((n) => !before.includes(n));
    expect(leaked, `leaked ${leaked.map((n) => basename(n)).join(', ')}`).toEqual([]);
  });
});
