...
describe('pr-inherit-issue-labels workflow', () => {
  it('copies the issue labels and drops the release-management ones', () => {
    const { posted } = run({
      PR_TITLE: 'fix(local): drop the phantom flag',
      PR_BODY: 'Closes #12',
  ...
  it('matches a full issue URL in THIS repo but not in another', () => {
    const mine = run({
      PR_TITLE: 'fix: x',
      PR_BODY: 'Resolves https://github.com/go-to-k/cdk-local/issues/33',
  ...
  it('posts to THIS PR\'s labels endpoint, not somewhere else', () => {
    ...
    expect(target).toContain('repos/go-to-k/cdk-local/issues/99/labels');
  ...
  it('finds a closing keyword that appears only in the title', () => {
    const { posted } = run({
      PR_TITLE: 'fix(local): drop the phantom flag, fixes #12',
