...
/**
 * Fixture stack for `cdkl start-service --watch` bind-mount source
 * fast path (Phase 4 of issue #214).
 *
...
export class LocalStartServiceWatchFastStack extends cdk.Stack {
  constructor(scope: Construct, id: string, props?: cdk.StackProps) {
    ...
    const cluster = new ecs.CfnCluster(this, 'Cluster', {
      clusterName: 'cdkl-start-service-watch-fast',
    });

    // Asset image built from the local `webapp/` directory. Editing
    // `webapp/server.cjs` flips the asset hash, triggering the watcher
    // reload. The Dockerfile sets `WORKDIR /app` and `COPY . /app/`
    // so `docker cp <new-asset-dir>/. <container>:/app/` lands the
    // edited source where the running Node process can `require` it
