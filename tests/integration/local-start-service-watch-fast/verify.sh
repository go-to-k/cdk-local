#!/usr/bin/env bash
# verify.sh — local-start-service-watch-fast integ test
# (Phase 4 of issue #214 — bind-mount source fast path)
#
# Exercises `cdkl start-service --watch`'s classifier dispatch against
...
#   3. Clean teardown on SIGTERM (no leftover cdkl-* containers /
#      networks).
#
...
${CDKL} start-service CdkLocalStartServiceWatchFastFixture:WebService \
  --watch \
  --no-pull \
