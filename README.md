...
- **`invoke-agentcore`** runs the agent container, waits for `GET /ping`, POSTs your `--event` to `POST /invocations`, and streams an SSE response live. A `customJwtAuthorizer` is enforced locally — pass `--bearer-token <jwt>` (verified against the runtime's OIDC discovery URL).
