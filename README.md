...
- **`start-cloudfront`** serves a CloudFront distribution's `viewer-request` -> origin -> `viewer-response` pipeline locally: it runs the distribution's CloudFront Functions (the same inline rewrite JS, in-process) over the origin content, so a URL-rewrite / routing / SPA-fallback change is verifiable in seconds instead of a deploy. Two origin kinds are served: an **S3 origin** (content resolved from the BucketDeployment source in the cloud assembly) and a **Lambda Function URL origin** (the backing Lambda is run locally via a real container — the same RIE machinery `invoke` uses — and invoked with the Function URL request/response shape, so a CDN-fronted Lambda is testable end to end). A pure-S3 distribution needs no Docker; a Function URL origin boots one Lambda container. Other custom origins (a generic HTTP backend) and Lambda@Edge are not run (warn-and-skip). `--origin <id>=<dir>` points an S3 origin at a local directory when the source can't be resolved automatically; `--tls` terminates real HTTPS; `--watch` re-synths + swaps the routing model on save.
