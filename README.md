...
cdkl start-agentcore MyStack/Agent             # serve an AgentCore Runtime warm: POST /invocations + GET /ping (repeatable) and the /ws session
...
- **`start-agentcore`** boots an HTTP / AGUI AgentCore Runtime container **once** and keeps it **warm**, serving the agent's contract on one port until `^C` — so you hit `POST /invocations` (+ `GET /ping`) **repeatedly against the same warm container** (vs single-shot `invoke-agentcore`, which boots + tears down per call), mirroring AgentCore's deployed warm-session model. Request body / SSE responses stream through, the boot-resolved `Authorization` is injected per request, and the same port also fronts the bidirectional `/ws` endpoint behind a host bridge that injects the session-id / `Authorization` upgrade headers a header-less browser `WebSocket` can't set ([details](docs/cli-reference.md#cdkl-start-agentcore-serve-an-agentcore-runtimes-http-contract--ws-locally)).
...
| Bedrock AgentCore Runtime agents | `invoke-agentcore` — container + `fromCodeAsset` / `fromS3` artifacts, HTTP / MCP / A2A / AGUI; `start-agentcore` — long-running warm serve of an HTTP / AGUI runtime (`POST /invocations` + `GET /ping` repeatable against one warm container, plus the `/ws` endpoint behind a header-injecting WebSocket bridge) |
