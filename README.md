...
cdkl start-cloudfront MyStack/SiteDist         # CloudFront distribution: S3 origin routing + Functions (static site / SPA)
...
- **`start-cloudfront`** serves a CloudFront distribution's `viewer-request` -> S3 origin -> `viewer-response` pipeline locally: it runs the distribution's CloudFront Functions (the same inline rewrite JS, in-process) over the S3 origin content resolved from the BucketDeployment source in the cloud assembly, so a URL-rewrite / routing / SPA-fallback change is verifiable in seconds instead of a deploy. Serves S3 origins only; custom origins (incl. a Lambda Function URL origin) and Lambda@Edge are not run (warn-and-skip) — run a Function URL standalone with `start-api`. `--origin <id>=<dir>` points an origin at a local directory when the source can't be resolved automatically; `--tls` terminates real HTTPS; `--watch` re-synths + swaps the routing model on save.
