...
- **Every session-wrap / task-complete report MUST end with a "Remaining
  ...
  **The full field reference — semantics, scales, labels, calibration and
  ...
  `next` needs one of that rule's three reasons** (a NEW integ fixture to
  write / external input / a COLD subsystem). Once the first two are
  excluded, the context test decides: if ANY file the fix touches or must
  read was read this session — a reviewer's read set counts — it is
  `now`; every recent wrap-time challenge on this flipped it. Before writing `next`, NAME the concrete command the next session
