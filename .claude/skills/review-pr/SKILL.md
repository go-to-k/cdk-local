...
3. **Compute bias factors** by scanning the `paths` list:
   ...
   **Down-bias triggers** (move tier DOWN by one step, never below inline) — only fires when ALL paths fall in the listed buckets:
   ...
   - **Pure docs/infra**: every path matches one of `.gitignore`, `README.md`, `**/*.md`, `docs/**`, `package.json` (top-level deps only — count as docs-ish for review purposes when the diff is dep bumps). The `**/*.md` entry catches markdown anywhere outside `docs/**` — most commonly integ-test READMEs (`tests/integration/*/README.md`) that are written for human readers but live under `tests/**`.
     - **Agent-instruction files are EXCLUDED from this bucket** (issue #501): `CLAUDE.md`, `.claude/CLAUDE.md`, and anything under `.claude/**` (skills, agents, hooks, rules, `settings*.json`) plus `.markgate.yml` used to be listed here, and they are markdown, so the `**/*.md` entry would re-admit them if the exclusion were dropped. A wrong rule in one of those files changes how EVERY future session behaves — the opposite of the low risk a down-bias assumes — so a skills-only or hooks-only PR keeps its size-derived tier. cdkd made the same change in its own copy; keep the two in sync, together with `DOWN_DOCS_REGEX` / `AGENT_INSTRUCTION_REGEX` in `.claude/hooks/pr-review-gate.sh`.
