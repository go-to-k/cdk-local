...
| 4 | Create no worktree; after confirming the tree is YOURS, branch IN PLACE off `origin/main` — ALWAYS, not only when the tree is detached or its PR has merged — and never commit onto `LAUNCH_BRANCH`. §5 carries the same rule and the recipe; this row is the version an orchestrator sees without opening that file | §5 |
