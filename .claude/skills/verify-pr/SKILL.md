...
12. **Residual review-nit sweep**
    ...
    - If NONE of (a) / (b) / (c) is true for any nit, FIX it — (a) is the default; (b) is reached only through a `next` reason written in the issue body. Do not set the `verify-pr` marker until every reviewer-flagged item is on one of those three paths.
