...
12. **Residual review-nit sweep**
    ...
    - For EACH item there, confirm ONE of the following is true BEFORE setting the `verify-pr` marker (same taxonomy as the session-wrap remaining-work buckets, go-to-k/cdkd#1257 — Fixed here / TODO / Won't-do):
      ...
      - (b) **TODO (issue #N)** — a GitHub issue exists AND this PR's body references it. This is the only bucket that leaves future work — and **(a) is the default, not (b)**: a nit a reviewer found lives in a file this session just reviewed, so the context test in `.claude/rules/session-report.md` makes it `now` unless one of that rule's three `next` reasons fires; file an issue only then, and write the reason in its body.
    ...
    - If NONE of (a) / (b) / (c) is true for any nit, FIX it — bucket (a) is the default; bucket (b) is reached only through one of `.claude/rules/session-report.md`'s three `next` reasons, written in the issue body. Do not set the `verify-pr` marker until every reviewer-flagged item is on one of those three paths.
