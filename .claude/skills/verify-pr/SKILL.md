...
12. **Residual review-nit sweep**
    ...
    - If NONE of (a) / (b) / (c) is true for any nit, FIX it — bucket (a) is the default; bucket (b) is reached only through one of `.claude/rules/session-report.md`'s two `next` reasons, written in the issue body. Do not set the `verify-pr` marker until every reviewer-flagged item is on one of those three paths.
