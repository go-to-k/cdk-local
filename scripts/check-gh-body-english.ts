 ...
 * ## The character class
 *
 * Identical to the PR-diff check's, and that is the whole of the detection
 * logic:
 *
 ...
 *
 * Kept character-for-character in sync with `scripts/check-pr-non-english-text.ts`,
 * whose subject is the PR DIFF rather than a body. The two share ONLY this
 * class; their subjects are disjoint. If you widen or narrow it here, widen or
 * narrow it there in the same change -- a class that differs between the two is
 * a body rule and a file rule that disagree, which is worse than either being
 * wrong. Fenced by `tests/unit/scripts/non-english-class-sync.test.ts`.
 *
 * Also carried over:
 *
 *   - the SCOPE. Only the published field is scanned, never a surrounding
 *     context: only `title` and `body` are read out of the subject. The
 *     author's login, the label names, a linked URL and the repository name
 *     are not text this repo published.
 ...
 *
 * ## Why an event and not a hook
 *
 * A PreToolUse gate would have to find the body inside a shell command: flag
 ...
 * segmentation and `cd` resolution. None of that exists here -- the event
 * carries the finished text -- so every shape a gate silently missed is
 * covered: a short `-b` / `-t` flag (which collides with `echo -n` /
 * `sort -t`), `gh api --input <file>`, `--body-file -` on stdin, a body the
 * shell assembled at run time, a gh call nested in a substitution, a subshell,
 * a loop or behind `xargs`, and the web UI, which no hook can reach at all.
 *
 * ## The one thing that is WEAKER, stated rather than discovered later
 *
 * TIMING. A hook refuses BEFORE the text reaches GitHub. This runs after: on a
 * PR the failing check blocks the merge, but on an issue or a comment the text
 * is already public and the check can only report it and ask for an edit.
 *
 * Release notes (`gh release create --notes` / `--notes-file`) are NOT covered:
 * they raise no `issues` / `issue_comment` / `pull_request` event. A release
 * body is written by release-please from commit subjects, which the PR-diff
 * check already sees.
 *
 ...
 * sibling PR-diff check has to test the identical set --
 * `tests/unit/scripts/non-english-class-sync.test.ts` fences this literal
 * against the one in `scripts/check-pr-non-english-text.ts`.
 *
 ...
 * PR-diff check reads as a violation -- the checker's own source would fail the
 * rule it enforces.
 */
