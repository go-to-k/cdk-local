 ...
 *     quotes, box-drawing characters in docs, arrow glyphs. Writing systems
 *     only.
 ...
 *   - The sidecar allow-list and its three load-bearing properties. See
 *     ALLOWLIST_PATH below.
 ...
 * hole in the English-only rule for that path, and a hole pointing at a
 * deleted file is a hole nobody can see. Drop the entry when the file goes.
 *
...
/**
 * The repo root, for resolving an allow-list ENTRY.
 *
 * Entries are repo-relative, and the staleness audit below has to resolve them
 * the same way whatever directory the check was invoked from. Resolving them
 * against `process.cwd()` instead makes every entry look stale — and therefore
 * fails the check — the moment it runs from anywhere but the repo root. That
 * is the fail-CLOSED direction, so it is a usability bug rather than a hole,
 * but it is still wrong: `scripts/` sits directly under the root.
 */
 ...
 * its own independent literal. The two are held identical by
