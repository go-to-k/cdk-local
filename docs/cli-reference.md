...
a non-hidden directory inside your project (the current directory, or the git
work tree holding the output directory) — and refused otherwise; it is REFUSED
at a container image's soft-reload source. BuildKit options
...
| `--from-cfn-stack [name]` | — | Read a deployed CloudFormation stack via `ListStackResources` and substitute `Ref` / `Fn::ImportValue` in env vars with the deployed physical IDs / exports, resolve a same-stack `AWS::ECR::Repository` ContainerUri to the deployed image, and resolve `AWS::SSM::Parameter::Value` env values (decrypted `SecureString` values are kept off the `docker run` argv; under `CDK_DOCKER=finch` on macOS / Windows they are refused unless `CDKL_ALLOW_SECRETS_ON_ARGV=1`, see [finch on macOS and Windows](local-emulation.md#finch-on-macos-and-windows)). Bare form uses the resolved stack name. |
...
## `cdkl start-agentcore` (serve an AgentCore Runtime's HTTP contract + `/ws` locally)
...
keeps it **warm**, serving the agent's contract on one host port until `^C`.

This mirrors AgentCore's deployed model, where many `InvokeAgentRuntime` calls
on the same `runtimeSessionId` reuse one warm microVM — vs single-shot
`invoke-agentcore`, which boots and tears down per call. So you can hit the
agent **repeatedly against the same warm container** instead of paying the
...
AgentCore `/ws` upgrade requires the
`X-Amzn-Bedrock-AgentCore-Runtime-Session-Id` header (and `Authorization`
under a `customJwtAuthorizer`); a browser `WebSocket` cannot set custom request
headers, so the bridge accepts a header-less client and opens a connection to
the container `/ws` with those headers injected — a fresh session-id UUID per
...
section), so on a machine whose only egress is a forward proxy, setting
those variables is what keeps the read out of this fallback -- a proxy that
is itself down, or one terminating TLS without `NODE_EXTRA_CA_CERTS`, still
lands here. The fallback emits a `warn` line
