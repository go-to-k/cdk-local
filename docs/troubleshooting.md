...
- **A scheme-less value means TLS *to the proxy*** — `HTTPS_PROXY=proxy.internal:3128` looks like a plain host:port, but the scheme of the *request* is prepended, not `http:`, so it resolves to `https://proxy.internal:3128` and an HTTPS target speaks TLS to the proxy itself. A plain HTTP forward proxy will not answer that, and the failure looks exactly like the one this section opens with. Write `HTTPS_PROXY=http://proxy.internal:3128` to get a `CONNECT` tunnel over plain HTTP.
