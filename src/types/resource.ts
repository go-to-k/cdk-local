...
/**
 * Context passed to provider `delete()`. cdk-local inlines the type here
 * for completeness — cdk-local itself never deletes resources (local
 * execution only), but `ResourceProvider` is still exported as a type
 * so callers that share state-schema types with deploy tools can read
 * the same `Record<string, ResourceState>` shape.
 */
