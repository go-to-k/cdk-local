...
export async function buildAgentCoreCodeImage(
  ...
  try {
    ...
    throw new LocalInvokeBuildError(
      `docker build failed for AgentCore code artifact (${displayUntrustedValue(options.sourceDir)})${stderr ? `: ${stderr}` : ''}`
    );
  } finally {
...
async function warnIfDependenciesNotVendored(
  ...
  if (isNode) {
    if (has('package.json') && !has('node_modules')) {
      logger.warn(
        `AgentCore code bundle ${displayUntrustedValue(sourceDir)} declares package.json but does not vendor node_modules. ` +
  ...
  if (manifest && !vendored) {
    ...
    logger.warn(
      `AgentCore code bundle ${displayUntrustedValue(sourceDir)} declares ${manifest} but does not vendor its dependencies. ` +
